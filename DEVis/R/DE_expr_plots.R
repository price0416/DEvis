...
de_diverge_plot <- function(res_list, filename="de_divergence_plot.pdf", theme=1, customLabels=FALSE, returnData=FALSE)
{
  ...
  #Prepare plot.
  plot_out <- ggplot(filteredList, aes(x=value, y=factor(variable, levels=as.character(unique(variable)), ordered=TRUE), label=variable)) + geom_point(stat='identity', aes(col=variable), size=3)
